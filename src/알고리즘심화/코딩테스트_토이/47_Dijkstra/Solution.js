function createGraphByList(num, edges) {
  ...
  edges.forEach(([src, dst, length]) => {
    // 무방향 그래프이므로 양쪽 방향 모두에 간선을 추가한다.
    _edges[src].push({ node: dst, length: length });
    _edges[dst].push({ node: src, length: length });
  });
  return _edges;
}
...
function Dijkstra(num, edges, start, end) {
  const newEdges = createGraphByList(num, edges);
  // newEdges = {1: [{ node: 2, weight: 6 }, { node: 3, weight: 2 }], 2: [], 3: [], 4:[] }...
  ...
  while (queue.length > 0) {
    ...
    newEdges[now].forEach(next => {
      if (dist[next.node] > dist[now] + next.length) {
        dist[next.node] = dist[now] + next.length;
